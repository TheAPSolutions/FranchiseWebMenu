...
@Component({
  selector: 'app-page-subcategories',
  templateUrl: './page-subcategories.component.html',
  styleUrl: './page-subcategories.component.scss'
})
export class PageSubcategoriesComponent {
  ...
  catchQuery(q: string) {
    if (q.length > 0) {
      this.SubCategoryService.searchCategories(q).subscribe({
        next: (data) => {
